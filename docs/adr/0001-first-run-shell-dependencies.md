...
|---|---|---|---|
| `expo-router` (+ its peers `react-native-screens`, `react-native-safe-area-context`) | navigation | apps/home-baker, packages/features; apps/pro-baker declares `react-native-safe-area-context` only (amended 2026-09-26), a peer of packages/ui that must link natively in each app | R1 |
...
| `react-native-svg` (amended 2026-09-23, coordinator ruling) | vector graphics | packages/ui; declared by apps/home-baker and apps/pro-baker (amended 2026-09-26), which link it natively | The corpus's tab and diet/allergen glyphs are SVG; redrawing them from views would be a worse deviation than one well-known renderer. Expo-supported (`npx expo install`) |
